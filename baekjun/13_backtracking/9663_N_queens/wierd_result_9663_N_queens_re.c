...
void 	check_uncheck_row(int N, int n_row, int status, int row[])
{
	row[n_row] = status;
}

void	check_uncheck_dia_des(int N, int n_row, int n_col, int status, int dia_des[])
{
	dia_des[n_col - n_row + (N - 1)] = status;
}

void	check_uncheck_dia_asc(int N, int n_row, int n_col, int status, int dia_asc[])
{
	dia_asc[n_col + n_row] = status;
}
...
void	check_uncheck(int N, int n_row, int n_col, int status, int row[], int dia_des[], int dia_asc[])
{
		...
		for(int i = 0; i < 2 * N - 1; i++)
			...
			printf("\n");
	check_uncheck_row(N, n_row, status, row);
	for(int i = 0; i < 2 * N - 1; i++)
			printf("(2)before check dia_asc[%d] = %d (2) dia_des[%d] = %d\n", i, dia_asc[i], i, dia_des[i]);
		printf("\n");

	check_uncheck_dia_des(N, n_row, n_col, status, dia_des);
		for(int i = 0; i < 2 * N - 1; i++)
			printf("(3)before check dia_asc[%d] = %d (3) dia_des[%d] = %d\n", i, dia_asc[i], i , dia_des[i]);
	printf("\n");


	check_uncheck_dia_asc(N, n_row, n_col, status, dia_asc);
		for(int i = 0; i < 2 * N - 1; i++)
			printf("(3)before check dia_asc[%d] = %d (3) dia_des[%d] = %d\n", i, dia_asc[i], i , dia_des[i]);
	printf("\n");
...
int main()
{
	...
	printf("resut = %d", ft_ten_queens_puzzle(N, row, dia_des, dia_asc));
}
