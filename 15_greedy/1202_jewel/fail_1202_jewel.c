...
// 종합하면 전체 시간복잡도는 max {O(N * log N), O(K * log K)}
...
/////////////////////////////////////////////////////////////
typedef int		t_bag;
typedef struct	s_jewel	t_jewel;
struct s_jewel{
	int M;
	int V;
};
...
// };
////////////////////////////////////////////////////////////////////
void		mergesort_bag(t_bag arr[], int start, int end);
void		merge_bag(t_bag arr[], int start, int mid, int end);
void		mergesort_jewel(t_jewel arr[], int start, int end);
void		merge_jewel(t_jewel arr[], int start, int mid, int end);

long long	find(t_jewel jewel[], t_bag bag[], int N, int K);

void		max_heapify(int	nod);
void		building_heap(void);
void		insert_heap(t_jewel to_put);
t_jewel		deque(void);

//////////////////////////////////////////////////////////////////
t_jewel		g_q_heap[300001];
int			g_q_length;
...
int main()
{
	...
	mergesort_jewel(jewel, 1, N);
	mergesort_bag(bag, 1, K);

	long long result = find(jewel, bag, N, K);
...
long long	find(t_jewel jewel[], t_bag bag[], int N, int K)
{
	...
	for (; (bag_i == 1) || (bag_i <= K && g_q_length > 0); bag_i++)
	{
		// printf("bag_i = %d, bag[bag_i] = %d\n", bag_i, bag[bag_i]);

		for (; jewel[jewel_i].M <= bag[bag_i] && jewel_i <= N; jewel_i++)
		{
			insert_heap(jewel[jewel_i]);
			// printf("   jewel_i = %d, jewel[jewel_i].M = %d, jewel[jewel_i].V = %d\n", jewel_i, jewel[jewel_i].M, jewel[jewel_i].V);
		}
		temp = deque();
		// printf("deque.V = %d, M = %d\n", temp.V, temp.M);
		ret += temp.V;
	}
	...
	// 	{
	// 		// printf("   for2 (k = %d)\n", k);
	// 		// printf("   (out)jewel[%d].M = %d, bag[%d].num = %d, bag[%d].visited = %d\n", n, jewel[n].M, k, bag[k].num, k, bag[k].visited);
	// 		if (jewel[n].M <= bag[k].num && bag[k].visited == false)
	// 		{
	// 			// printf("   if\n");
	// 			ret += jewel[n].V;
	// 			bag[k].visited = true;
	// 			// printf("      jewel[%d].V = %d, bag[%d].visited = true\n", n, jewel[n].V, k);
	// 			break ;
	// 		}
	// 		// printf("\n");
	// 	}
...
void	mergesort_bag(t_bag arr[], int start, int end)
{
	...
	mergesort_bag(arr, start, mid);
	mergesort_bag(arr, mid + 1, end);
	merge_bag(arr, start, mid, end);
}
...
void	mergesort_jewel(t_jewel arr[], int start, int end)
{
	...
	int mid = (start + end) / 2;
// printf("here");
	mergesort_jewel(arr, start, mid);
	mergesort_jewel(arr, mid + 1, end);
	merge_jewel(arr, start, mid, end);
}

void	merge_jewel(t_jewel arr[], int start, int mid, int end)
{
	t_jewel max; max.M = INT_MAX;
	t_jewel temp1[300001];
	t_jewel temp2[300001];
// printf("here");
	int i = start;
	for (; i <= mid; i++)
	{
